...
const ServerStatus = () => {
  ...
  // Add missing loadMoreOnlinePlayers function
  const loadMoreOnlinePlayers = useCallback(async () => {
    ...
    try {
      ...
      if (playersResponse.ok) {
        ...
        // Check 3 more batches to find additional online players
        const batchSize = 150;
        const additionalBatches = 3;
        const currentPlayerNames = onlinePlayers.map(p => p.name);
        const newOnlinePlayers: any[] = [];
        
        for (let batch = 0; batch < additionalBatches; batch++) {
          // Start from a different offset to get different players than initial fetch
          const startIndex = (10 + batch) * batchSize;
          const endIndex = Math.min(startIndex + batchSize, shuffledPlayers.length);
          
          if (startIndex >= shuffledPlayers.length) break;
          ...
          try {
            ...
            if (batchResponse.ok) {
              ...
              const batchOnline = batchData.filter((player: any) => 
                ...
                !currentPlayerNames.includes(player.name) // Don't include duplicates
              );
  
              newOnlinePlayers.push(...batchOnline);
              console.log(`Batch ${batch + 1}: Found ${batchOnline.length} new online players`);
            }
  
            // Delay between requests to avoid rate limiting
            await new Promise(resolve => setTimeout(resolve, 300));
          } catch (error) {
            console.log(`Additional batch ${batch + 1} failed:`, error);
          }
        }
  
        if (newOnlinePlayers.length > 0) {
          const processedNewPlayers = newOnlinePlayers.map((player: any) => ({
            name: player.name,
          ...
          console.log(`Successfully added ${processedNewPlayers.length} new online players`);
        } else {
          console.log('No new online players found in additional batches');
        }
      }
    } catch (error) {
  ...
  }, [onlinePlayers, isStaffMember, getPlayerRank]);
