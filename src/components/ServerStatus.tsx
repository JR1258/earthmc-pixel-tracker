                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 max-h-80 overflow-y-auto">
                  {regularPlayers.slice(0, displayLimit).map((player, index) => (
                    <div
                      ...
                      className="flex flex-col items-center p-3 bg-blue-900/20 rounded-lg border border-blue-500/20 hover:bg-blue-900/30 transition-colors"
                    >
                      <div className="w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center mb-2">
                        <User className="w-4 h-4 text-white" />
                      </div>
                      <div className="text-center">
                        <div className="font-semibold text-blue-300 text-sm truncate w-full">
                          {player.name}
                        </div>
                        {(player.town || player.nation) && (
                          <div className="text-xs text-gray-400 truncate w-full">
                            {player.town && <span>{player.town}</span>}
                            {player.nation && player.town && <span> • </span>}
                            {player.nation && !player.town && <span>{player.nation}</span>}
                          </div>
                        )}
                      </div>
